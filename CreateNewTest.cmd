...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^ "$lines = @(   'using System;','using System.Net.Http;','using System.Net.Http.Headers;','using System.Text;','using System.Text.Json;','using System.Threading;','using System.Threading.Tasks;','','','public class TestProcedure','{','    private readonly HttpClient _http;','    private readonly JsonSerializerOptions _json = new(JsonSerializerDefaults.Web);','    private string? _token;','','    // Normal instance constructor with parameter','    public TestProcedure(HttpClient? httpClient = null)','    {','        _http = httpClient ?? new HttpClient { BaseAddress = new Uri(\"https://qa-cortex.nayax.com/\") };','    }','','    public static async Task Main(string[] args)','    {','        var proc = new TestProcedure();   // Instantiate','        await proc.RunAsync();','    }','','    public async Task RunAsync()','    {','        try','        {','            string token = await SignIn(\"sergeyr\", \"rubi69qa1******\");','            Console.WriteLine($\"Received token in Sign in: {token}\");','        }','        catch (Exception ex)','        {','            Console.WriteLine($\"Sign-in failed: {ex.Message}\");','        }','','        try','        {','            if (string.IsNullOrEmpty(_token))','            {','				throw new Exception(\"Token is null, test is aborted.\"); ','            }','            else ','			{','                string token = await NextTestStep(_token);','            }','        }','        catch (Exception ex)','        {','            Console.WriteLine($\"Function failed: {ex.Message}\");','        }','','        _http.Dispose();','    }','','    public async Task<string> SignIn(string username, string password, CancellationToken ct = default)','    {','        var url = \"users/v1/signin\";','        var payload = new { username, password };','        var body = new StringContent(JsonSerializer.Serialize(payload, _json), Encoding.UTF8, \"application/json\");','','        using var rsp = await _http.PostAsync(url, body, ct).ConfigureAwait(false);','        rsp.EnsureSuccessStatusCode();','','        var json = await rsp.Content.ReadAsStringAsync(ct).ConfigureAwait(false);','        using var doc = JsonDocument.Parse(json);','        var tokenProp = doc.RootElement.GetProperty(\"token\");','','        _token = tokenProp.GetString();','        if (string.IsNullOrEmpty(_token))','            throw new InvalidOperationException(\"Token not found in sign-in response.\");','','        _http.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue(\"Bearer\", _token);','        return _token;','    }','','    public async Task<string> NextTestStep(string token)','    {','        await EnsureSignedIn();','        Console.WriteLine($\"Received token in the Next Test Step: {token}\");','        return token;','    }','','    private async Task<string> EnsureSignedIn()','    {','        if (string.IsNullOrWhiteSpace(_token))','            throw new InvalidOperationException(\"Not signed in. Call SignIn first.\");','        return _token;','    }','}'   ); Set-Content -Path 'TestProcedure.cs' -Value $lines -Encoding UTF8"
